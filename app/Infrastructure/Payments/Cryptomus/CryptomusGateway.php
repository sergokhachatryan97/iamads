...
final class CryptomusGateway implements PaymentGatewayInterface
{
    public function __construct(
        private CryptomusClient $client,
    ...
    public function initiate(PaymentIntent $intent): PaymentInitiationResult
    {
        ...
        if ($state !== 0) {
            ...
            throw new RuntimeException("Cryptomus payment create failed: {$err}");
        }

        $result = $response['result'] ?? null;
        if (!is_array($result)) {
            throw new RuntimeException('Cryptomus API: missing result');
        }

        $uuid = $result['uuid'] ?? null;
        $url  = $result['url'] ?? null;
        if (!$uuid || !$url) {
            throw new RuntimeException('Cryptomus API: result missing uuid or url');
        }
    ...
    {
        $status = (string) ($body['status'] ?? $body['payment_status'] ?? '');
        $status = strtolower(trim($status));

        return match ($status) {
            'paid', 'paid_over'                                                                          => PaymentStatus::PAID,
            'process', 'check', 'confirm_check', 'wrong_amount', 'wrong_amount_waiting'                 => PaymentStatus::PENDING,
