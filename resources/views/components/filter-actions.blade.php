...
<div class="flex flex-col sm:flex-row gap-2 mt-auto pt-2" @if($closeOnClick) @click.stop @endif>
    <button
        type="submit"
        @click="$dispatch('close-filter-dropdown'); $dispatch('filter-applied')"
    ...
    @if($clearRoute)
        <a
            ...
            class="flex-1 inline-flex justify-center items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-1 focus:ring-offset-1 focus:ring-indigo-500 transition-colors"
        >
            {{ __($clearText) }}
        </a>
    @endif
